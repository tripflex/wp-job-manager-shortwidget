...
class Widget_submit_job_form extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'submit_job_form-id',
			__( 'Submit Job Form', 'wp-job-manager-shortwidget' ),
			array(
				'description'	=>	__( 'Output the submit a job form.', 'wp-job-manager-shortwidget' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'submit_job_form');		
	...
	public function register_admin_styles_scripts() {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/submit_job_form-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-submit_job_form.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-submit_job_form.php';		
		}else{
			return;
		}
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'submit_job_form', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_submit_job_form");' ) );
