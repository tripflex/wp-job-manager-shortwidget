...
class Widget_job_field extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'job_field-id',
			__( 'Job Field', 'wp-job-manager-shortwidget' ),
			array(
				'description'	=>	__( 'Output a specific field, custom or default, for a job listing..', 'wp-job-manager-shortwidget' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'job_field');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/job_field-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-job_field.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-job_field.php';		
		}else{
			return;
		}
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'job_field', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_job_field");' ) );
