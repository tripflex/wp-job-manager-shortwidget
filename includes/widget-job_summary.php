...
class Widget_job_summary extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'job_summary');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/job_summary-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-job_summary.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-job_summary.php';		
		}else{
			return;
		}
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'job_summary', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_job_summary");' ) );
