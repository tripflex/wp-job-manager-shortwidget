...
class Widget_jobs extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'jobs-id',
			__( 'Job List', 'wp-job-manager-shortwidget' ),
			array(
				'description'	=>	__( 'Output job listings.', 'wp-job-manager-shortwidget' )
			)
		,array( 'width' => 495)
	...
	public function widget( $args, $instance ) {
		...
		echo $element->render_element($instance, '', 'jobs');		
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/jobs-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-jobs.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-jobs.php';		
		}else{
			return;
		}
		...
		foreach ($configfiles as $key=>$fieldfile) {
			...
			echo "<div class=\"wp-job-manager-shortwidget-widget-config-nav\">\r\n";
			echo "	<ul>\r\n";
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'jobs', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_jobs");' ) );
